                ...
                if any(i in item_list for i in ["1.01", "2.01", "5.02", "2.05", "7.01"]):
                    ...
                    try:
                        ...
                        if res.status_code in [200, 206]:
                            ...
                            if "2.05" in item_list and not has_processed_8k_205:
                                messages = [
                                    {"role": "system", "content": (
                                        "You are a financial analyst. Analyze this SEC 8-K Item 2.05 (Exit or Disposal Activities) text. "
                                        "Extract a short summary (max 8 words) of what is being closed or how many employees are laid off. "
                                        "You MUST write your response strictly in RUSSIAN language. Do not use English words.\n"
                                        "EXAMPLE:\n"
                                        "Input text: 'The company announced a plan to close 10 stores and lay off 150 employees.'\n"
                                        "Your output: Закрытие 10 филиалов и сокращение 150 сотрудников\n\n"
                                        "If no specific restructuring details are found, respond ONLY with NO."
                                    )},
                                    {"role": "user", "content": f"Text: {clean_text[:5000]}"}
                                ]
                                ans = clean_llm_response(call_ollama(messages, max_tokens=50, temperature=0.0))
                                if ans and "NO" not in ans.upper() and len(ans) > 2:
                                    qual_point_text = f"Официальный запуск программы реструктуризации (Item 2.05) (+20 баллов): {ans}"
                                    if qual_point_text not in qual_points:
                                        qual_score += 20
                                        qual_points.append(qual_point_text)
                                        is_ma = True
                                        reasons.append("Реструктуризация (Item 2.05)")
                                        desc = f"План реструктуризации: {ans}" if not desc else desc + f" | План: {ans}"
                                        has_processed_8k_205 = True
