...
def fetch_sec_ma_hints(client, ticker):
    ...
    try:
        ...
        if transcript_url:
            ...
            # Отсекаем Q&A сессию, оставляем только Prepared Remarks
            ...
            clean_transcript = text[:6000] # Лимит токенов
            
            messages = [
                {"role": "system", "content": "You are a financial analyst. Read the earnings call prepared remarks. Extract the exact name of any major turnaround strategy or restructuring plan mentioned by the CEO (e.g., 'Bold New Chapter'). If found, respond ONLY with the strategy name. If no such strategy is mentioned, respond ONLY with NO. CRITICAL: Output ONLY the requested data. No chain of thought, no reasoning, no intro text."},
                {"role": "user", "content": f"Ticker: {ticker}. Transcript: {clean_transcript}"}
            ]
            ans = clean_llm_response(call_groq_api_with_fallback(messages, max_tokens=50, temperature=0.0))
    ...
    try:
        ...
        for i in range(len(forms)):
            ...
            # Исключаем отчеты старше последнего года
            ...
            if form in ["SC 13D", "SC 13D/A"]:
                ...
                headers = {
                    "User-Agent": client.headers.get("User-Agent", "ValuationDashboardBot/3.0 (dikam@example.com)")
                }
                try:
                    ...
                    if res_13d.status_code == 200:
                        ...
                        if item4_text:
                            ...
                            # Использование интеллектуального движка инференса Gemma 3 4B
                            ...
                            intent_scores = {
                                "HOSTILE_TAKEOVER": 30,
                                "ACTIVIST_RESTRUCTURING": 25,
                                "FRIENDLY_TOEHOLD": 15,
                                "PASSIVE_INVESTMENT": 5
                            }
                            add_score = intent_scores.get(intent_type, 5)
            ...
            # ─── НАЧАЛО ВСТАВКИ: Шаг 1. Разделение типов раскрытий (13G) ───
                ...
                # Зануляем требования и цитаты, так как пассивная форма их не содержит
                key_demands_val = []
                evidence_quotes_val = []
                
                # Даем минимальный качественный буст (+5) за появление крупного опорного акционера
                qual_score += 5
            ...
            # ─── КОНЕЦ ВСТАВКИ ──────────────────────────────────────────────
                ...
                is_ma = True
                reasons.append(f"Прокси-заявление о слиянии ({form})")
                
                # ─── ИНТЕЛЛЕКТУАЛЬНЫЙ АНАЛИЗАТОР ФОРМЫ 14A ───────────────────
                cik_str = str(meta.get("cik", "")).zfill(10)
                acc_clean = acc.replace("-", "")
                doc_url = f"https://www.sec.gov/Archives/edgar/data/{cik_str}/{acc_clean}/{pdoc}"
                headers = {
                ...
                try:
                    ...
                    if res_14a.status_code == 200:
                        ...
                        # Декодируем и очищаем текстовый слой документа
                        ...
                        # Ищем Summary Term Sheet или раздел The Merger, где лежат условия
                        idx = clean_text.lower().find("summary term sheet")
                        if idx == -1:
                            idx = clean_text.lower().find("the merger agreement")
                        if idx == -1:
                            idx = clean_text.lower().find("proposed merger")
                        if idx == -1:
                            idx = 0
                            
                        # Вырезаем плотный смысловой кусок текста для контекстного окна
                        ...
                        messages = [
                            {
                                "role": "system", 
                                "content": (
                                    "You are an expert M&A analyst. Read the merger proxy statement (14A) text.\n"
                                    "Identify the ACQUIRER/BUYER company full name and the exact DEAL TERMS per share (e.g., $15.50 in cash, or exchange ratio of stock).\n"
                                    "Respond strictly in Russian in the following clean format:\n"
                                    "Слияние с [Имя Покупателя] | Условия: [Условия на русском языке]\n"
                                    "Do not add any markdown, quotes, preambles or explanations. Just one single string output."
                                )
                            },
                            {"role": "user", "content": f"Target Ticker: {ticker}. Text: {chunk}"}
                        ]
                        
                        # Вызываем ротатор моделей (Ollama / Groq)
                        ans = call_groq_api_with_fallback(messages, max_tokens=120, temperature=0.0)
                        ans_cleaned = clean_llm_response(ans)
                        
                        if ans_cleaned and len(ans_cleaned) > 5 and "NO" not in ans_cleaned.upper():
                            desc = ans_cleaned
                            # Существенный качественный буст к скору за официальное подписание Definitive Agreement
                            qual_score += 35
                        else:
                            desc = f"Анонсировано окончательное слияние ({form}), детали условий обрабатываются."
                except Exception as e:
                ...
                try:
                    ...
                    if res_m.status_code in [200, 206]:
                        ...
                        merger_prompt = (
                            "You are a strict M&A financial analyst. Read the cover page of this Merger Proxy statement.\n"
                            "Identify the Acquirer Name (the company acquiring the target) and the Deal Terms/Consideration.\n"
                            "You MUST respond with a single valid JSON object ONLY.\n\n"
                            "REQUIRED JSON FORMAT:\n"
                            "{\n"
                            "  \"acquirer_name\": \"Name of Acquirer (in Russian or English)\",\n"
                            "  \"deal_terms\": \"Brief summary of terms in Russian (max 12 words, e.g., $17.50 за акцию наличными or 0.5 акций Покупателя)\"\n"
                            "}"
                        )
                        merger_data = gemma_4b_inference_engine(clean_text[:8000], system_instruction=merger_prompt)
                        acq = merger_data.get("acquirer_name", "Партнер по слиянию")
                        terms = merger_data.get("deal_terms", "Условия не определены")
                except Exception as e:
                    print(f"    [!] Error parsing merger proxy cover for {ticker}: {e}")
                
                reasons.append(f"Анонсировано слияние с {acq} ({form})")
                desc = f"Слияние с {acq} | Условия: {terms}"
                qual_score += 90
                qual_points.append(f"Анонсировано слияние с {acq} ({form}) на условиях '{terms}' (+90 баллов)")
                ...
                try:
                    ...
                    if res_m.status_code in [200, 206]:
                        ...
                        merger_prompt = (
                            "You are a strict M&A financial analyst. Read the cover page of this S-4 Registration Statement.\n"
                            "Identify the Acquirer Name (the company issuing shares) and the Deal Terms/Consideration.\n"
                            "You MUST respond with a single valid JSON object ONLY.\n\n"
                            "REQUIRED JSON FORMAT:\n"
                            "{\n"
                            "  \"acquirer_name\": \"Name of Acquirer (in Russian or English)\",\n"
                            "  \"deal_terms\": \"Brief summary of terms in Russian (max 12 words, e.g., обмен акций с коэффициентом 0.5)\"\n"
                            "}"
                        )
                        merger_data = gemma_4b_inference_engine(clean_text[:8000], system_instruction=merger_prompt)
                        acq = merger_data.get("acquirer_name", "Покупатель акций")
                        terms = merger_data.get("deal_terms", "Обмен акций")
                except Exception as e:
                    print(f"    [!] Error parsing S-4 cover for {ticker}: {e}")
                
                reasons.append(f"Регистрация акций для слияния с {acq} (S-4)")
                desc = f"Обмен акций с {acq} | Условия: {terms}"
                qual_score += 90
                qual_points.append(f"Регистрация акций для слияния с {acq} (S-4) на условиях '{terms}' (+90 баллов)")
                ...
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        # 1. Takeover Defenses Check (DePamphilis Ch 3)
                        if form in ["10-K", "DEF 14A"]:
                            defenses = {
                                "poison pill": "Ядовитая пилюля (Poison Pill)",
                                "shareholder rights plan": "План прав акционеров (Poison Pill)",
                                "staggered board": "Шахматный совет директоров (Staggered Board)",
                                "classified board": "Классифицированный совет директоров (Classified Board)",
                                "golden parachute": "Золотой парашют (Golden Parachute)",
                                "change-in-control": "Золотой парашют (Golden Parachute)",
                                "change in control": "Золотой парашют (Golden Parachute)",
                                "supermajority to approve merger": "Требование сверхбольшинства для одобрения слияний (Supermajority for Mergers)",
                                "supermajority for merger": "Требование сверхбольшинства для одобрения слияний (Supermajority for Mergers)",
                                "supermajority to amend charter": "Требование сверхбольшинства для изменения устава (Supermajority for Charter Amendments)",
                                "supermajority for charter amendments": "Требование сверхбольшинства для изменения устава (Supermajority for Charter Amendments)",
                                "supermajority to amend bylaws": "Требование сверхбольшинства для изменения bylaws (Supermajority for Bylaw Amendments)",
                                "special meeting": "Ограничение на созыв внеочередных собраний (Limits on Special Meetings)",
                                "written consent": "Ограничение на письменные согласия (Limits on Written Consent)"
                            }
                            found_defenses = []
                            for kw, label in defenses.items():
                                if kw in doc_text_lower:
                                    found_defenses.append(label)
                            
                            if found_defenses:
                                unique_defenses = list(set(found_defenses))
                                detected_defenses.extend(unique_defenses)
                                qual_score -= 20
                                qual_points.append(f"Обнаружены защиты от поглощения (-20 баллов): {', '.join(unique_defenses)}")
                                is_ma = True
                                reasons.append("Защиты от поглощения (Charter/Bylaws)")
                                desc = f"Защитные барьеры: {', '.join(unique_defenses)}" 

                        # 2. Turnaround / Restructuring Check (MD&A)
                        if form in ["10-K", "10-Q"]:
                            mda_text = extract_mda_section(res.text, is_10k=(form == "10-K"))
                            if mda_text:
                                mda_excerpt = mda_text[:12000]
                                mda_system_instruction = (
                                    "You are a strict M&A financial analyst. Analyze the provided MD&A periodic filing (10-K/10-Q).\n"
                                    "Look ONLY for explicit mentions of: restructuring plans, store closures, layoffs, or turnaround strategies.\n"
                                    "Look ONLY for explicit segment carve-outs or spin-offs. Ignore standard financial tables and revenues.\n"
                                    "If none of these are present, you MUST set turnaround_detected/spinoff_detected to false and summary to \"\".\n"
                                    "Respond with a single valid JSON object ONLY in Russian language. No other languages allowed.\n\n"
                                    "REQUIRED JSON FORMAT:\n"
                                    "{\n"
                                    "  \"intent\": \"PASSIVE_INVESTMENT\",\n"
                                    "  \"summary\": \"Краткое описание (макс 8 слов на РУССКОМ языке). Если ничего нет, напишите пустую строку \"\"\",\n"
                                    "  \"demands\": [],\n"
                                    "  \"quotes\": [],\n"
                                    "  \"turnaround_detected\": true_or_false,\n"
                                    "  \"spinoff_detected\": true_or_false\n"
                                ...
                                mda_data = gemma_4b_inference_engine(mda_excerpt, system_instruction=mda_system_instruction)
                                
                                if mda_data.get("turnaround_detected"):
                                    qual_score += 20
                                    ans = mda_data.get("summary", "Программа оздоровления бизнеса")
                                    qual_points.append(f"Стратегия Turnaround в {form} MD&A (+20 баллов): {ans}")
                                    is_ma = True
                                ...
                                if mda_data.get("spinoff_detected"):
                                    ...
                                    qual_points.append(f"Segment Reporting / Возможный Spin-off в {form} (+15 баллов): {ans_seg}")
                                    is_ma = True
                                    reasons.append("Сегментная реструктуризация / Carve-out")
                                    if desc:
                                        desc += f" | Реструктуризация сегментов: {ans_seg}"
                                    else:
                                        desc = f"Реструктуризация сегментов: {ans_seg}" 
                except Exception as e:
                ...
                if any(i in item_list for i in ["1.01", "2.01", "5.02", "2.05", "7.01"]):
                    # Для 8-K делаем быстрый Range-запрос первых 35 КБ, чтобы проверить на M&A контекст
                    cik_str = str(meta.get("cik", "")).zfill(10)
                    acc_clean = acc.replace("-", "")
                    doc_url = f"https://www.sec.gov/Archives/edgar/data/{cik_str}/{acc_clean}/{pdoc}"
                    ...
                    try:
                        ...
                        if res.status_code in [200, 206]:
                            # ── Баг 1: Очистка кодировочных артефактов ──────────────────────────
                            # SEC EDGAR часто отдаёт Windows-1252 или HTML-сущности (smart quotes,
                            # em-dash), которые после strip_tags превращаются в Unicode-глитчи.
                            import html
                            import unicodedata
                            raw_text = res.content.decode('utf-8', errors='replace')
                            # 1. Декодируем HTML-сущности: &mdash; → —, &rdquo; → ", etc.
                            raw_text = html.unescape(raw_text)
                            # 2. Удаляем HTML-теги
                            clean_text = re.sub(r'<[^>]*>', ' ', raw_text)
                            # 3. Заменяем типографские и Windows-1252 спецсимволы на ASCII-аналоги
                            clean_text = clean_text.replace('\u2019', "'").replace('\u2018', "'")
                            clean_text = clean_text.replace('\u201c', '"').replace('\u201d', '"')
                            clean_text = clean_text.replace('\u2014', ' - ').replace('\u2013', ' - ')
                            clean_text = clean_text.replace('\u2026', '...')
                            # 4. Нормализуем Unicode: удаляем combining characters (тип Mn)
                            # Именно они дают глитч ̶ (Combining Long Stroke Overlay)
                            clean_text = ''.join(
                                c for c in unicodedata.normalize('NFKD', clean_text)
                            ...
                            positive_kws = ["merger", "acquisition", "takeover", "purchase agreement", "combination agreement", "tender offer"]
                            negative_kws = ["credit agreement", "credit facility", "revolving credit", "office lease", "lease agreement", "underwriting agreement", "employment agreement", "severance agreement", "indenture"]
                            
                            has_positive = any(pk in lower_text for pk in positive_kws)
                            has_negative = any(nk in lower_text for nk in negative_kws)
                            
                            is_ma_kws = has_positive and not has_negative
                            
                            # ── Снайперские фильтры качественных факторов ──
                            if "5.02" in item_list:
                                try:
                                    messages = [
                                        {"role": "system", "content": "You are a financial analyst. Read the 8-K excerpt. Determine if a new board member was added due to activist pressure or a cooperation agreement. If yes, respond ONLY with the activist fund's name. If no, respond ONLY with NO. CRITICAL: Output ONLY the requested data. No chain of thought, no reasoning, no intro text."},
                                        {"role": "user", "content": f"Ticker: {ticker}. Text: {clean_text[:5000]}"}
                                    ]
                                    ans = clean_llm_response(call_groq_api_with_fallback(messages, max_tokens=50, temperature=0.0))
                                    if ans and "NO" not in ans.upper() and len(ans) > 2:
                                            qual_score += 25
                                            qual_points.append(f"Приход активиста в Совет Директоров (Item 5.02) (+25 баллов): {ans}")
                                            is_ma = True
                                            reasons.append("Соглашение с активистом (8-K 5.02)")
                                            desc = f"Активист в СД: {ans}" 
                                except Exception:
                                    pass
                                    
                            if "2.05" in item_list or "7.01" in item_list:
                                try:
                                    messages = [
                                        {"role": "system", "content": "You are a financial analyst. Read the 8-K excerpt. Identify if the company is restructuring, closing stores, or announcing a turnaround strategy like 'Bold New Chapter'. If yes, extract a short summary (max 10 words). If no, respond ONLY with NO. CRITICAL: Output ONLY the requested data. No chain of thought, no reasoning, no intro text."},
                                        {"role": "user", "content": f"Ticker: {ticker}. Text: {clean_text[:5000]}"}
                                    ]
                                    ans = clean_llm_response(call_groq_api_with_fallback(messages, max_tokens=50, temperature=0.0))
                                    if ans and "NO" not in ans.upper() and len(ans) > 2:
                                            qual_score += 20
                                            item_name = "Item 2.05" if "2.05" in item_list else "Item 7.01"
                                            qual_points.append(f"Официальный запуск программы реструктуризации ({item_name}) (+20 баллов): {ans}")
                                            is_ma = True
                                            reasons.append(f"Реструктуризация ({item_name})")
                                            if desc:
                                                desc += f" | План: {ans}"
                                            else:
                                                desc = f"План реструктуризации: {ans}" 
                                except Exception:
                                    pass

                            # Если это не сделка 1.01/2.01, но найдены качественные триггеры (is_ma = True), то оставляем. Иначе пропускаем.
                            if not (("1.01" in item_list or "2.01" in item_list) and is_ma_kws):
                                if not is_ma:
                                    continue
                                
                            # Use Groq API to determine if the company is ACQUIRER or TARGET
                            is_target = True
                            idx_for_llm = -1
                            for pk in positive_kws:
                                idx_for_llm = lower_text.find(pk)
                                if idx_for_llm != -1: break
                            
                            if idx_for_llm != -1:
                                llm_context = clean_text[max(0, idx_for_llm - 500):min(len(clean_text), idx_for_llm + 500)]
                                try:
                                    messages = [
                                        {"role": "system", "content": "You are a financial analyst. Read the 8-K excerpt and determine if the filing company is the ACQUIRER (buying another company) or the TARGET (being bought). Respond with ONLY the word ACQUIRER or TARGET. CRITICAL: Output ONLY the requested data. No chain of thought, no reasoning, no intro text."},
                                        {"role": "user", "content": f"Filing company ticker: {ticker}. Text: {llm_context}"}
                                    ]
                                    answer = call_groq_api_with_fallback(messages, max_tokens=10, temperature=0.0).upper()
                                    if answer:
                                        print(f"    [LLM-CLASSIFY] {ticker} {date} -> LLM answer: {answer}")
                                        if "ACQUIRER" in answer:
                                            is_target = False
                                except Exception as e:
                                    print(f"    [LLM-CLASSIFY] Exception: {e}")
                            
                            if not is_target:
                                # ── Баг 2: Фильтр "мёртвых сделок" ─────────────────────────────
                                # Если компания — покупатель (ACQUIRER) И это Item 2.01 (сделка
                                # завершена) И с даты прошло > 90 дней → помечаем как исторический
                                # артефакт и пропускаем (не засоряем текущий анализ).
                                from datetime import datetime, timedelta, timezone
                                try:
                                    filing_dt = datetime.strptime(date, "%Y-%m-%d")
                                    days_old = (datetime.now(timezone.utc).replace(tzinfo=None) - filing_dt).days
                                except:
                                    days_old = 0
                                is_closed_deal = "2.01" in item_list and days_old > 90
                                if is_closed_deal:
                                    print(f"    [~] SEC: {ticker} {date} — ACQUIRER + Item 2.01 + {days_old}d → [HISTORICAL_ACQUISITION], пропускаем.")
                                    continue  # Архивная закрытая сделка — не интересна
                                print(f"    [~] SEC: {ticker} {date} — ACQUIRER + {item_list}, пропускаем (просто покупатель).")
                                continue  # Просто покупатель без закрытой сделки — тоже пропускаем
                                
                            # Если M&A маркер найден, извлекаем сниппет для описания
                            is_ma = True
                            if "1.01" in item_list:
                                reasons.append("Существенное соглашение (8-K Item 1.01)")
                            if "2.01" in item_list:
                                reasons.append("Завершение сделки (8-K Item 2.01)")
                                
                            # Вырезаем предложение вокруг ключевого слова
                            snippet = ""
                            for pk in positive_kws:
                                idx = lower_text.find(pk)
                                if idx != -1:
                                    start = max(0, idx - 45)
                                    end = min(len(clean_text), idx + 105)
                                    snippet = clean_text[start:end].replace("\n", " ").strip()
                                    snippet = re.sub(r'\s+', ' ', snippet)
                                    # Экранируем кавычки для безопасного HTML-вывода
                            ...
                            if snippet:
                                ...
                                desc = "Обнаружено M&A соглашение (8-K)"
                                
                    except Exception as ex:
                        # При сетевой ошибке НЕ ставим is_ma=True — это создаёт ложные срабатывания.
                        # Пропускаем этот filing и продолжаем.
                        print(f"    [!] SEC: сетевая ошибка при чтении 8-K: {ex}. Пропускаем.")
                        is_ma = False
            ...
            if is_ma:
                ...
                ma_filings.append({
                    ...
                    "description": desc,
                    "url": doc_url,
                    "key_demands": key_demands_val,
                    "evidence_quotes": evidence_quotes_val
                })
                if len(ma_filings) >= 4:
                    break
    except Exception as e:
            ...
            try:
                ...
                if inst is not None and not inst.empty:
                    ...
                    ACTIVIST_KEYWORDS = ["ELLIOTT", "JANA", "STARBOARD", "SINGER", "COHN", "BARINGTON", 
                                         ...
                                         "VALUE ACT", "ANCORA", "LAND AND BUILDINGS", "GREENLIGHT"]
                    ...
                    # Сопоставляем активных держателей с раскрытыми намерениями из SC 13D
                    for h in active_holders:
                        h_name_upper = h["name"].upper()
                        for hint in sec_ma_hints:
                            if hint["form"] in ["SC 13D", "SC 13D/A"]:
                                ...
                                if is_match:
                                    ...
                                    if "13G" not in hint["form"]:
                                        # Повышаем тип инвестора до Активиста
