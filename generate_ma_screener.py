    ...
    try:
        ...
        for i in range(len(forms)):
            ...
            if form in ["SC 13D", "SC 13D/A"]:
                # Защита от дублирования: обрабатываем только самую свежую (текущую) версию декларации
                if "13D_ACTIVE" in processed_ownership_forms:
                    continue
                is_ma = True
                ...
                try:
                    ...
                    if res_13d.status_code == 200:
                        ...
                        if item4_text:
                            ...
                            # ─── РАСЧЕТ ВРЕМЕННОГО ЛАГА (Time Decay) ───
                            try:
                            ...
                            processed_ownership_forms.add("13D_ACTIVE")
                except Exception as e:
                    print(f"    [!] Error parsing 13D for {ticker}: {e}")
                    
            elif form in ["SC 13G", "SC 13G/A"]:
                # Защита от дублирования: берем только самую актуальную форму, игнорируя старые архивные поправки
                if "13G_PASSIVE" in processed_ownership_forms:
                    continue
                is_ma = True
                ...
                processed_ownership_forms.add("13G_PASSIVE")
