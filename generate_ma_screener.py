                                ...
                                messages = [
                                    {
                                        "role": "system", 
                                        "content": (
                                            "You are an expert financial analyst. Analyze this SEC 8-K Item 5.02 text (Appointment/Election of Directors).\n"
                                            "Determine if a new director or board member was appointed as a result of an agreement, pressure, or settlement with an investor or private equity firm (e.g., Silver Lake, Elliott, etc.).\n"
                                            "If it is a routine corporate appointment by the company itself without investor pressure, set 'is_fund_appointment' to false.\n"
                                            "If it is an investor or fund appointment, extract the full English name of the fund/investor and provide a clean Russian translation.\n\n"
                                            "You MUST respond with a single valid JSON object ONLY containing exactly three keys: 'is_fund_appointment', 'fund_name_en', and 'fund_name_ru'.\n"
                                            "REQUIRED JSON FORMAT:\n"
                                            "{\n"
                                            "  \"is_fund_appointment\": true_or_false,\n"
                                            "  \"fund_name_en\": \"Full English name of the fund (e.g., Silver Lake Group)\",\n"
                                            "  \"fund_name_ru\": \"Имя фонда на русском (транслитерация, например, Сильвер Лейк Груп)\"\n"
                                            "}\n"
                                            "Do not add markdown blocks or text preambles. Output pure JSON only."
                                        )
                                    },
                                ...
                                if ans:
                                    ...
                                    if json_match:
                                        try:
                                            ...
                                            if data_502.get("is_fund_appointment"):
                                                fund_en = data_502.get("fund_name_en", "").strip()
                                                fund_ru = data_502.get("fund_name_ru", "").strip()
                                                
                                                # Программный заслон от галлюцинаций малой модели
                                                if fund_ru and fund_ru.lower() not in ["да", "нет", "изолира", "реструктуризация"] and len(fund_en) > 3:
                                                    qual_point_text = f"Приход представителя фонда в Совет Директоров (Item 5.02) (+25 баллов): {fund_ru}"
                                                    if qual_point_text not in qual_points:
                                                        qual_score += 25
                                                        qual_points.append(qual_point_text)
                                                        is_ma = True
                                                        reasons.append(f"Назначение от {fund_en} (8-K 5.02)")
