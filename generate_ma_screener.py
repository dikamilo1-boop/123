    ...
    try:
        ...
        for i in range(len(forms)):
            ...
            if form in ["SC 13D", "SC 13D/A"]:
                        ...
                        messages = [
                            {
                                ...
                                "content": (
                                    ...
                                    "Identify the ACQUIRER/BUYER company full name and the exact DEAL TERMS per share.\n"
                                    "CRITICAL REQUIREMENT: You MUST translate both the acquirer name and all deal terms strictly into RUSSIAN language. Do not leave English terms or corporate names untranslated.\n"
                                    "Example: 'Amneal Pharmaceuticals, Inc.' -> 'Амнил Фармасьютикалс, Инк.'\n"
                                    "Example: 'Exchange of Class A common stock' -> 'Обмен обыкновенных акций Класса А'\n\n"
                                    "You MUST respond with a single valid JSON object ONLY containing exactly two keys: 'acquirer_name' and 'deal_terms'.\n"
                                    "REQUIRED JSON FORMAT:\n"
                                    "{\n"
                                    "  \"acquirer_name\": \"Имя компании покупателя НА РУССКОМ ЯЗЫКЕ\",\n"
                ...
                qual_point_text = f"Анонсировано слияние с {acq} ({form}) на условиях '{terms}' (+90 баллов)"
                ...
                try:
                    ...
                    if res_m.status_code in [200, 206]:
                        ...
                        messages = [
                            {
                                ...
                                "content": (
                                    ...
                                    "Identify the Acquirer Name and the Deal Terms/Consideration. Translate EVERYTHING into Russian.\n"
                                    "You MUST respond with a single valid JSON object ONLY containing exactly two keys: 'acquirer_name' and 'deal_terms'.\n\n"
                                    "REQUIRED JSON FORMAT:\n"
                                    "{\n"
                                    "  \"acquirer_name\": \"Имя эмитента/покупателя НА РУССКОМ\",\n"
                                    "  \"deal_terms\": \"Условия обмена акций НА РУССКОМ\"\n"
                                    "}"
                                )
                            },
                        ...
                        if ans:
                            ...
                            if json_match:
                                merger_json = json.loads(json_match.group(0))
                                acq = merger_json.get("acquirer_name", "Покупатель акций")
                ...
                qual_point_text = f"Регистрация акций для слияния с {acq} (S-4) на условиях '{terms}' (+90 баллов)"
                if qual_point_text not in qual_points:
                    qual_score += 90
                    qual_points.append(qual_point_text)
                has_processed_merger = True
                    ...
                    try:
                        ...
                        if res.status_code in [200, 206]:
                            ...
                            if "5.02" in item_list:
                                messages = [
                                    {"role": "system", "content": "You are a financial analyst. Determine if a new board member was added due to activist pressure. If yes, respond ONLY with the activist fund's name IN RUSSIAN (transliterated). If no, respond ONLY with NO."},
                                    {"role": "user", "content": f"Text: {clean_text[:5000]}"}
                                ]
                                ans = clean_llm_response(call_ollama(messages, max_tokens=50, temperature=0.0))
                                if ans and "NO" not in ans.upper() and len(ans) > 2:
                                    qual_point_text = f"Приход активиста в Совет Директоров (Item 5.02) (+25 баллов): {ans}"
                                    if qual_point_text not in qual_points:
                                        qual_score += 25
                                        qual_points.append(qual_point_text)
                                        is_ma = True
                            ...
                            if "2.05" in item_list or "7.01" in item_list:
                                messages = [
                                    {"role": "system", "content": "You are a financial analyst. Identify if the company is restructuring, closing stores, or announcing an acquisition. If yes, extract a short summary (max 10 words) STRICTLY IN RUSSIAN language. If no, respond ONLY with NO."},
                                    {"role": "user", "content": f"Text: {clean_text[:5000]}"}
                                ]
                                ans = clean_llm_response(call_ollama(messages, max_tokens=50, temperature=0.0))
                                if ans and "NO" not in ans.upper() and len(ans) > 2:
                                    item_name = "Item 2.05" if "2.05" in item_list else "Item 7.01"
                                    qual_point_text = f"Официальный запуск программы реструктуризации ({item_name}) (+20 баллов): {ans}"
                                    # СТРОГАЯ ДЕДУПЛИКАЦИЯ: Добавляем только уникальные тексты событий
                                    if qual_point_text not in qual_points:
                                        qual_score += 20
                                        qual_points.append(qual_point_text)
                                        is_ma = True
                                        reasons.append(f"Реструктуризация ({item_name})")
                                        desc = f"План реструктуризации: {ans}" if not desc else desc + f" | План: {ans}"
