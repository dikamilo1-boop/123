...
def gemma_4b_inference_engine(prompt_text, system_instruction=None):
    ...
    # Сверхплаский промпт-шаблон, адаптированный под возможности модели 4B
    default_system = (
        "You are a strict M&A financial analyst. Analyze the provided SEC text.\n"
...
def extract_item4_section(html_content):
    """
    Метод точечного парсинга: извлекает только раздел Purpose of Transaction (Item 4)
    из формы Schedule 13D.
    """
    ...
    m4 = item4_regex.search(html_content)
    if not m4:
        # Фоллбэк regex с мягкими условиями
        fallback_regex_4 = re.compile(r"item\s*4\s*[\s\S]{0,100}purpose\s*of", re.IGNORECASE)
        m4 = fallback_regex_4.search(html_content)
    ...
    if m4:
        ...
        if m5:
            item4_chunk = item4_chunk[:m5.start()]
            
        soup = BeautifulSoup(item4_chunk, "lxml")
        clean_text = soup.get_text(separator="\n")
        
        # Нормализуем пробелы и Юникод
...
def strip_boilerplate(text):
    ...
    («may acquire from time to time...») с помощью регулярных выражений.
    """
    import re
    if not text:
        return ""
    ...
    boilerplates = [
        ...
        r"may from time to time[\s\S]{0,50}acquire[\s\S]{0,50}additional shares",
        r"may from time to time[\s\S]{0,50}dispose of[\s\S]{0,50}some or all",
    ...
    try:
        ...
        for i in range(len(forms)):
            ...
            # ─── КОНЕЦ ВСТАВКИ ──────────────────────────────────────────────
                ...
                is_ma = True
                reasons.append("Регистрация акций для слияния (S-4)")
                        ...
                        if res.status_code in [200, 206]:
                            ...
                            if idx_for_llm != -1:
                                ...
                                try:
                                    ...
                                    if answer:
                                        print(f"    [DEBUG-GROQ] {ticker} {date} -> LLM answer: {answer}")
