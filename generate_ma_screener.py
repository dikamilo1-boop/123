    ...
    try:
        ...
        for i in range(len(forms)):
            ...
            # ─── КОНЕЦ ВСТАВКИ ──────────────────────────────────────────────
                ...
                # ─── ЛОКАЛЬНЫЙ ИНТЕЛЛЕКТУАЛЬНЫЙ АНАЛИЗАТОР ФОРМЫ 14A (БЕЗ GROQ) ───
                cik_str = str(meta.get("cik", "")).zfill(10)
                acc_clean = acc.replace("-", "")
                doc_url = f"https://www.sec.gov/Archives/edgar/data/{cik_str}/{acc_clean}/{pdoc}"
                headers = {
                ...
                try:
                    ...
                    if res_14a.status_code in [200, 206]:
                        import html
                        import unicodedata
                        
                        # Декодируем и очищаем текстовый слой документа от тегов
                        ...
                        # Находим блок Summary Term Sheet, где прописаны условия
                        ...
                        messages = [
                            {
                                ...
                                "content": (
                                    ...
                                    "You MUST respond with a single valid JSON object ONLY containing exactly two keys: 'acquirer_name' and 'deal_terms'.\n"
                                    "Translate properties into clear Russian language.\n\n"
                                    "REQUIRED JSON FORMAT:\n"
                                    "{\n"
                                    "  \"acquirer_name\": \"Полное имя компании покупателя\",\n"
                                    "  \"deal_terms\": \"Краткие условия сделки на русском (например, $21.00 за акцию наличными)\"\n"
                                    "}\n"
                                    "Do not add any preambles, markdown blocks or explanations. Output pure JSON only."
                                )
                            },
                            {"role": "user", "content": f"Target Ticker: {ticker}. Text: {chunk}"}
                        ]
                        
                        # Вызываем строго локальный инференс Ollama (Gemma 3 4B)
                        ...
                        if ans:
                            ...
                            if json_match:
                                ...
                                acq = merger_json.get("acquirer_name", "Партнер по слиянию")
                                terms = merger_json.get("deal_terms", "Условия не определены")
                except Exception as e:
                    print(f"    [!] Ошибка при локальном парсинге прокси-формы 14A для {ticker}: {e}")
