...
def simulate_negative_covenants(post_trans_debt, post_trans_ebitda, net_income, capex, da, buyer_type="financial"):
    ...
    # ─── ЮРИДИЧЕСКИЙ ЗАСЛОН ДЛЯ ОТРИЦАТЕЛЬНОГО/НУЛЕВОГО EBITDA ───
    if post_trans_ebitda <= 0:
        leverage = 999.9  # Безопасный численный индикатор критического плеча для корректной работы JSON
        dividend_allowed = False
        dividend_cap_pct = 0.0
        div_reason = f"Заблокировано ковенантом: операционный поток (EBITDA) Newco принимает деструктивное отрицательное или нулевое значение (${post_trans_ebitda/1e6:.1f}M). Выплата дивидендов категорически запрещена."
        
        allowed_debt = 0.0
        debt_headroom = 0.0
        additional_debt_allowed = False
        debt_reason = f"Доп. заимствования полностью заблокированы: EBITDA Newco находится в дефиците (${post_trans_ebitda/1e6:.1f}M). Свободный кредитный лимит (Headroom) отсутствует."
    else:
        ebitda = post_trans_ebitda
        leverage = debt / ebitda

        # 1. Dividend Restrictions
        dividend_allowed = True
        dividend_cap_pct = 100.0
        div_reason = "Разрешено в полном объеме (долговая нагрузка объединенной компании находится в пределах нормы)."

        if buyer_type == "financial":
            if leverage > 4.5:
                dividend_allowed = False
                dividend_cap_pct = 0.0
                div_reason = f"Заблокировано ковенантом: плечо Newco {leverage:.2f}x превышает жесткий лимит LBO 4.5x. Выплата дивидендов запрещена."
            elif leverage > 3.5:
                dividend_allowed = True
                dividend_cap_pct = 15.0
                div_reason = f"Ограничено ковенантом: плечо Newco {leverage:.2f}x находится в критическом диапазоне [3.5x - 4.5x]. Распределение прибыли ограничено лимитом 15% от Net Income."
        else:  # strategic
            if leverage > 3.5:
                dividend_allowed = False
                dividend_cap_pct = 0.0
        ...
        if not additional_debt_allowed:
            debt_reason = f"Доп. заимствования полностью заблокированы: плечо Newco {leverage:.2f}x превышает жесткий предел финансового соглашения ({max_leverage_limit:.1f}x)."

    # 3. Capex Limits
    capex_limit = max(da_val * 1.2, post_trans_ebitda * 0.15)
    ...
    if capex_breach:
        capex_reason = (
            ...
            f"Запланированная модернизация под угрозой: требуется предварительное согласие синдиката кредиторов ⚠️."
        )
...
# 3. Precedent Transactions (Comparable Recent Transactions)
            ...
            t_country = yf_info.get("country") or "United States" if yf_info else "United States"
            ...
            if is_biotech_dcf_override:
                ...
                biotech_note = (
                    ...
                    f"Pipeline Premium для Big Pharma M&A, а не on DCF-дисконте."
                )
            ...
            # Инициализация профиля целевой компании для алгоритма Buyer Matchmaker
            target_profile = {
                "ticker": t,
                "sic": sic_code,
