...
def check_whale_catalysts(influential_shareholders, sec_ma_hints=None):
    """
    Интеллектуальный верификатор китов: активирует триггер времени сделки только тогда,
    когда присутствие фонда подтверждено свежей активностью (SEC-репортами или докупкой в 13F).
    Применяет закон Уильямса (Williams Act) и защиту от временного лага (Time Decay > 12 месяцев).
    """
    qual_points = []
    qual_score = 0
    from datetime import datetime
    
    # Шаг А: Собираем текущий юридический статус каждого фонда на основе хронологии SEC с учетом Time Decay
    whale_status = {}
    if sec_ma_hints:
        for hint in sec_ma_hints:
            form_upper = str(hint.get("form", "")).upper()
            desc_upper = str(hint.get("description", "")).upper()
            reason_upper = str(hint.get("reason", "")).upper()
            h_date = hint.get("date", "")
            
            try:
                age_days = (datetime.now() - datetime.strptime(h_date, "%Y-%m-%d")).days
            except:
                age_days = 0
                
            for whale_core_name in MA_DRIVERS_REGISTRY.keys():
                if whale_core_name in desc_upper or whale_core_name in reason_upper:
                    if age_days > 365:
                        # События старше 12 месяцев теряют силу активного катализатора (Time Decay)
                        if whale_core_name not in whale_status:
                            whale_status[whale_core_name] = "STALE"
                    else:
                        if "13D" in form_upper:
                            whale_status[whale_core_name] = "ACTIVE"
                        elif "13G" in form_upper and whale_status.get(whale_core_name) != "ACTIVE":
                            whale_status[whale_core_name] = "PASSIVE"

    # Шаг Б: Формируем текстовый пул для перекрестной проверки только из свежих отчетов (<= 365 дней)
    sec_text_pool = ""
    if sec_ma_hints:
        for hint in sec_ma_hints:
            h_date = hint.get("date", "")
            try:
                ...
                age_days = 0
            if age_days <= 365:
                sec_text_pool += " " + str(hint.get("description", "")).upper()
                sec_text_pool += " " + str(hint.get("reason", "")).upper()
            
    for holder in influential_shareholders:
        holder_name = holder.get('name', '').upper().replace('.', '')
        pct_change = holder.get('pct_change', 0.0)
        
        for whale_core_name, whale_data in MA_DRIVERS_REGISTRY.items():
            if whale_core_name in holder_name:
                status = whale_status.get(whale_core_name, "UNKNOWN")
                
                # ЮРИДИЧЕСКИЙ ЗАСЛОН + TIME DECAY:
                # Если фонд официально пассивен по 13G или полностью устарел (>12 месяцев без свежих 13D),
                # мы полностью блокируем агрессивный триггер подготовки к LBO/выкупу.
                if status in ["PASSIVE", "STALE"] and pct_change <= 0:
                    continue
                    
                is_verified_by_sec = whale_core_name in sec_text_pool if sec_text_pool else False
                is_recent_accumulation = pct_change > 0.0
                
                if is_verified_by_sec or is_recent_accumulation:
                    # Дисконтирование баллов: если позиция пассивная, но есть недавний добор по 13F, снижаем балл влияния
                    if status == "PASSIVE":
                        qual_points.append(f"Вход {whale_core_name.title()}: Накопление доли по 13F, но инвестор сохраняет пассивный статус по форме 13G.")
                        qual_score += max(5, whale_data['impact_score'] - 15)
                    else:
...
class MATimingEngine:
    ...
    def calculate_timing(self, ticker, parsed_data, wacc_data, yf_info, sec_ma_hints, cache, turnaround_news=None, buyback_suspended=None, whale_points=None, whale_score=0, qual_points=None, qual_score=0):
        ...
        # 1. Activist Presence (Form 13D/A) с учетом Time Decay
        has_13d = False
        for hint in (sec_ma_hints or []):
            if "13D" in str(hint.get("form", "")):
                try:
                    from datetime import datetime
                    h_days = (datetime.now() - datetime.strptime(hint.get("date", ""), "%Y-%m-%d")).days
                    # Окно возможностей открыто строго в течение 12 месяцев после подачи формы
                    if h_days <= 365:
                        has_13d = True
                        break
                except:
        ...
        if has_13d:
            score += 25
            points.append("Инвестор-активист вошел в капитал (подана свежая форма SEC 13D в течение последнего года) — открыто активное окно до сделки/реструктуризации.")
...
def fetch_sec_ma_hints(client, ticker):
    ...
    try:
        ...
        for i in range(len(forms)):
            ...
            # ─── ДИФФЕРЕНЦИРОВАННЫЙ ВРЕМЕННОЙ ФИЛЬТР (Уничтожение слепой зоны целей) ───
            # Для ключевых форм раскрытия структуры капитала (13D/G) копаем на 3 года назад.
            # Для текущих новостей, реструктуризаций и слияний (8-K, 14A, 10-K) оставляем актуальный 1 год.
            ...
            if form in ["SC 13D", "SC 13D/A"]:
                ...
                try:
                    ...
                    if res_13d.status_code == 200:
                        # Защита от дублирования по конкретному фонду: берем только самую свежую версию
                        clean_text_13d = res_13d.text.upper()
                        target_whale = "GENERIC"
                        for whale_core_name in MA_DRIVERS_REGISTRY.keys():
                            if whale_core_name in clean_text_13d:
                                target_whale = whale_core_name
                                break
                        if f"13D_{target_whale}" in processed_ownership_forms:
                            continue
                        processed_ownership_forms.add(f"13D_{target_whale}")
                        ...
                        if item4_text:
                            ...
                            if age_days > 365:
                                qual_points.append(f"Архивный инвестор-активист (13D {intent_type}) (+0 баллов, аннулировано по Time Decay > 12 месяцев): {explanation}")
                            else:
                ...
                fund_name_found = "институционального инвестора"
                try:
                    res_13g = requests.get(doc_url, headers=headers, timeout=10)
                    if res_13g.status_code == 200:
                        import html
                        raw_text_13g = html.unescape(res_13g.content.decode('utf-8', errors='replace'))
                        clean_text_13g = re.sub(r'<[^>]*>', ' ', raw_text_13g).upper()
                        
                        target_whale = "GENERIC"
                        for whale_core_name in MA_DRIVERS_REGISTRY.keys():
                            if whale_core_name in clean_text_13g:
                                fund_name_found = f"фонда {whale_core_name.title()}"
                                target_whale = whale_core_name
                                break
                                
                        # Защита от дублирования по конкретному фонду: пропускаем старые архивные поправки
                        if f"13G_{target_whale}" in processed_ownership_forms:
                            continue
                        processed_ownership_forms.add(f"13G_{target_whale}")
                        
                        reasons.append(f"Пассивное накопление доли >5% ({fund_name_found}) (SC 13G)")
                        intent_type = "PASSIVE_INVESTMENT"
                        explanation = f"Раскрытие пассивной доли {fund_name_found} в Schedule 13G без целей изменить контроль."
                        ...
                        if age_days > 365:
                            qual_points.append(f"Архивный пассивный инвестор (13G {intent_type}) (+0 баллов, аннулировано по Time Decay > 12 месяцев): {explanation}")
                        else:
                            qual_score += 5
                    ...
                    print(f"    [!] Ошибка сканирования текста 13G для {ticker}: {e}")
    ...
    for t in deep_tickers:
        ...
        try:
            ...
            try:
                ...
                if inst is not None and not inst.empty:
                    ...
                    for _, row in inst.head(20).iterrows():
                        ...
                        # Резервируем статус активиста исключительно под верифицированные раскрытия 13D
                        if is_strategic:
            ...
            try:
                ...
                # ─── СИНХРОНИЗАЦИЯ 13F ДЕКЛАРАЦИЙ (Устранение противоречия на дашборде) ───
                for h in active_holders:
                    h_name_upper = h["name"].upper()
                    for whale_core_name, whale_data in MA_DRIVERS_REGISTRY.items():
                        if whale_core_name in h_name_upper and h.get("pct_change", 0.0) > 0.0:
                            already_in_sec = any(whale_core_name in str(hint.get("description", "")).upper() or whale_core_name in str(hint.get("reason", "")).upper() for hint in sec_ma_hints)
                            if not already_in_sec:
                                from datetime import datetime
                                sec_ma_hints.append({
                                    "date": datetime.now().strftime("%Y-%m-%d"),
                                    "form": "13F (Квартальный)",
                                    "reason": f"Накопление доли фондом {h['name']}",
                                    "description": f"По данным последней декларации SEC Form 13F фонд увеличил позицию на +{h['pct_change']:.2f}%. Количество: {(h['shares']/1e6):.2f}M шт. (${{h['value']}}M)",
                                    "url": "https://www.sec.gov/edgar/searchedgar/companysearch",
                                    "key_demands": [],
                                    "evidence_quotes": []
                                })
                
                # Передаем актуализированный sec_ma_hints для валидации китов
                whale_points, whale_score = check_whale_catalysts(active_holders, sec_ma_hints)
...
# 3. Precedent Transactions (Comparable Recent Transactions)
            ...
            # ─── МАТРИЦА ИНФЕРЕНСА ПРОКСИ-УСПЕХА (Bebchuk E-Index & Readiness Matrix) ───
            ...
            if e_index_score == 0:
                if avi >= 100:
                    proxy_success_prob = {
                        "pressure_success": "95%+",
                        "proxy_fight_win": "85%",
                        "reason": "Полная беззащитность уставных документов (E-Index = 0/6): У цели отсутствуют Poison Pill (ядовитая пилюля) и Staggered Board (шахматный совет). Это означает, что активисты могут полностью сменить весь состав Совета директоров за один электоральный цикл на ближайшем годовом собрании акционеров. Отсутствие ограничений на созыв внеочередных собраний (Action Limits): Активисты могут инициировать процедуру письменного согласия (Consent Solicitation) в любое время года, форсируя голосование без ожидания годового собрания. Экстремально распыленный капитал: Менеджмент не имеет «опорных» голосов для защиты. Институциональные инвесторы при отрицательном спреде эффективности (ROC < WACC) гарантированно проголосуют за альтернативных директоров, предложенных активистом. Развязка конфликта: На практике при AVI 100/100 дело редко доходит до открытого голосования акционеров. Совет директоров цели добровольно выделит активистам 1–2 кресла в обмен на Standstill Agreement."
                    }
                else:
                    proxy_success_prob = {
                        "pressure_success": "75% - 85%",
                        "proxy_fight_win": "65% - 75%",
                        "reason": "Крайне низкий (нулевой) уровень уставной защиты и maximal уязвимость к поглощению (E-Index = 0/6). У компании отсутствуют ядовитые пилюли, шахматный совет или ограничения на созыв собраний. У умеренная вероятность немедленного исхода прокси-борьбы обусловлена исключительно отсутствием текущих острых катализаторов времени (низкий/средний AVI скор готовности). Исход будет зависеть от способности активиста мобилизовать крупных индексных про Providers (Vanguard, BlackRock), которые по умолчанию поддерживают смену контроля при нулевых барьерах."
                    }
            elif e_index_score <= 1:
                if avi >= 70:
                    proxy_success_prob = {
                        "pressure_success": "80%+",
                        "proxy_fight_win": "70%",
                    ...
                    proxy_success_prob = {
                        "pressure_success": "65% - 75%",
                        "proxy_fight_win": "55% - 65%",
                        "reason": "Минимальный уровень уставной защиты (E-Index = 1/6). Корпоративные барьеры слабы, однако фундаментальные катализаторы времени не активированы в полной мере. Успех поглощения или давления будет сильно зависеть от позиции прокси-советников (ISS / Glass Lewis) на годовом собрании."
                    }
                ...
                proxy_success_prob = {
                    ...
                    "reason": f"Умеренный уровень уставной защиты (E-Index = {e_index_score}/6) и средняя уязвимость. Исход прокси-борьбы будет сильно зависеть от позиции крупных институциональных инвесторов (Vanguard, BlackRock, etc.)."
                }
            depamphilis_metrics = {
            ...
            alpha = 0.50
            has_13d = any("13D" in str(hint.get("form", "")).upper() for hint in (sec_ma_hints or []))
            avi = timing_profile.get("score", 0) if timing_profile else 0
            if has_13d:
                if avi >= 100:
                    alpha = 0.75
            ...
            # [4] Hubris & Winner's Curse Filter (Fiduciary Ceiling по Депамфилису, Гл. 14)
            fiduciary_ceiling = opt_price + pv_synergy_per_share
            is_hubris_trap = False
            cvr_earnout_required_per_share = 0.0
            
            if offer_price > fiduciary_ceiling:
                is_hubris_trap = True
                cvr_earnout_required_per_share = offer_price - fiduciary_ceiling
                offer_price = fiduciary_ceiling

            # ─────────────────────────────────────────────────────────────────
            # 5. ДВИЖОК ОПЕРАЦИОННОЙ СИНЕРГИИ (Operating Synergy Score)
            # ─────────────────────────────────────────────────────────────────
            ...
            sga_excess = sga_ratio - sector_sga_median
            if sga_excess >= 0.10:
                score_sga = 90
            ...
            # ─────────────────────────────────────────────────────────────────
            # 6. КОМПИЛЯЦИЯ АУДИТОРСКИХ ПРЕДУПРЕЖДЕНИЙ И РЕАЛИСТИЧНОСТИ
            # ─────────────────────────────────────────────────────────────────
            audit_points = []
            ...
            if is_biotech_dcf_override:
                ...
                biotech_note = (
                    ...
                    f"успеха клинических испытаний (pNPV). Скор основан на Pipeline Premium."
                )
                audit_points.append(biotech_note)

            if is_hubris_trap:
                hubris_note = (
                    f"⚠️ Ловушка синергии (Synergy Trap): Предложение на основе рыночной цены с премией (${price * 1.30:.2f}) "
                    f"превышает предельную экономическую ценность актива под вашим контролем (${fiduciary_ceiling:.2f}). "
                    f"Выявлен критический риск 'Проклятия победителя' (Winner's Curse). Твердая цена оферты принудительно "
                    f"ограничена фидуциарным потолком, а избыток в размере ${cvr_earnout_required_per_share:.2f} на акцию "
                    f"требует выноса в условные обязательства (CVR / Earnouts)."
                )
                audit_points.append(hubris_note)
            ...
            if buyback_suspended:
                buyback_status = f"Отмена выкупа: несмотря на исторический выкуп на {-shares_change_recent:.1f}% за год, менеджмент приостановил программу buyback."
            elif shares_change_recent < -0.5:
                buyback_status = f"Активная защита: менеджмент проводит регулярный обратный выкуп акций (buyback на {-shares_change_recent:.1f}% за последний год)."
            elif cash_ratio < 0.03:
                buyback_status = f"Отсутствие выкупа: количество акций не снижается, а запас свободных средств на балансе минимален."
            else:
                if is_financial:
                    buyback_status = f"Специфика сектора: свободный кэш выступает в качестве регуляторного буфера, а не избыточного капитала."
                else:
                    buyback_status = f"Отсутствие выкупа: количество акций не снижается, свободный кэш накапливается без распределения."
            audit_points.append(buyback_status)

            lbo_feasible = False
            is_reit = wacc_data.get("sector") == "Real Estate"
            if is_reit:
                lbo_status = "Секторная специфика: оценка LBO через классический EV/EBITDA неприменима для REIT."
                lbo_feasible = False
            elif is_negative_ebitda:
                target_buyer_desc = "Big Pharma" if is_biotech_profile else "отраслевых игроков"
                lbo_status = f"Долговой LBO невозможен: EBITDA отрицательная ({ebitda/1e6:.0f}M). Сценарий сделки: стратегическое поглощение за кэш ({target_buyer_desc})."
                lbo_feasible = False
                ...
                lbo_status = f"Текущая высокая долговая нагрузка (Debt/EBITDA = {debt_ebitda:.1f}x) исчерпывает кредитный лимит для LBO."
                lbo_feasible = False
            elif debt_ebitda is not None and debt_ebitda > 3.0:
                lbo_status = f"Умеренно-высокий долг (Debt/EBITDA = {debt_ebitda:.1f}x) ограничивает потенциал кредитного плеча."
                lbo_feasible = True
            elif ev_ebitda is not None and debt_ebitda is not None and ev_ebitda > 0 and ev_ebitda < 8.0 and debt_ebitda < 2.0:
                lbo_status = f"Низкая оценка (EV/EBITDA = {ev_ebitda:.1f}x) и свободный баланс делают компанию идеальной мишенью для финансового LBO."
                lbo_feasible = True
            else:
                lbo_status = f"Финансовая структура умеренно пригодна для LBO (EV/EBITDA = {ev_ebitda:.1f}x, Debt/EBITDA = {debt_ebitda:.1f}x)."
                lbo_feasible = True
            audit_points.append(lbo_status)

            # Финальный расчет веса
            ...
            is_short_attack = ((short_pct_float > 0.20) or (short_ratio_days > 10.0 and short_pct_float > 0.10) or ("hindenburg" in str(buyout_news).lower()))
            short_pct = short_pct_float
            is_critical_tech = (parsed.get("industry") or yf_info.get("industry") or "").lower() in ["semiconductors", "biotechnology", "aerospace & defense"]

            regulatory_blocked = False
            if mcap > 100_000_000_000:
                reg_status = f"Огромная рыночная капитализация (${mcap/1e9:.1f} млрд) гарантирует пристальный антимонопольный контроль."
                regulatory_blocked = True
                audit_points.append(reg_status)
            elif wacc_data["sector"] in ["Utilities", "Energy", "Financial Services"]:
                reg_status = f"Компания относится к регулируемому сектору ({wacc_data['sector']}), что повышает риск отмены сделки."
                regulatory_blocked = True
                audit_points.append(reg_status)
                
            is_agreed_deal = False
            if buyout_news:
                audit_points.append(f"Обнаружен активный процесс поглощения в новостях: '{buyout_news}'.")
                is_agreed_deal = True
            if antitrust_news or is_hhi_block:
                audit_points.append(f"Обнаружены регуляторные риски в новостях (FTC/антимонопольный иск).")
                regulatory_blocked = True
            if activist_news:
                audit_points.append(f"Обнаружено активистское давление в новостях: '{activist_news}'.")
                
            is_china_risk = t_country in ("China", "Hong Kong")
            if is_china_risk:
                audit_points.append("Внимание: Компания использует структуру VIE. Выкуп со стороны западных фондов заблокирован.")
            ...
            elif is_hubris_trap and score_underval < 30:
                defense_verdict, defense_verdict_class, feasibility_score = "НИЗКАЯ РЕАЛИСТИЧНОСТЬ (Проклятие победителя / Hubris Trap)", "text-red", 30
                weighted_score = min(30, weighted_score)
            elif is_hhi_block:
            ...
            for other_t, other_obj in cache.get("tickers", {}).items():
                ...
                if ofin.get("sector") != wacc_data.get("sector"): continue
                o_revenue = ofin.get("revenue")
                if not o_revenue: continue
                industry_universe_revenue.append(o_revenue)
                o_ebitda = ofin.get("ebitda", 0.0)
                o_debt = ofin.get("debt", 0.0)
                if o_ebitda > 0 and o_debt / o_ebitda > 4.0: continue
                strategic_bidders.append({
                    "ticker": other_t, "buyer_type": "strategic", "sic": ofin.get("sic"),
                    "sector": ofin.get("sector"), "revenue": o_revenue, "sga": ofin.get("sga", 0.0),
                    "ebitda": o_ebitda, "debt": o_debt, "cash": ofin.get("cash", 0.0),
                    "mcap": (ofin.get("mcap", 0.0) or 0.0) * 1e9, "wacc": (ofin.get("wacc", 10.0) or 10.0) / 100.0,
                    "net_income": ofin.get("net_income", 0.0), "capex": ofin.get("capex", 0.0), "da": ofin.get("da", 0.0),
                })

            generic_pe_sponsor = {
                "ticker": "GENERIC_PE_SPONSOR", "buyer_type": "financial", "sic": None, "sector": None,
                "revenue": 0.0, "sga": 0.0, "ebitda": 0.0, "debt": 0.0, "cash": mcap * 0.30, "mcap": 0.0, "wacc": 0.10,
            }

            buyer_matches = run_buyer_matchmaker(target_profile, strategic_bidders + [generic_pe_sponsor], industry_universe=industry_universe_revenue)
            buyer_profile = {"strategic": buyer_matches[:10], "financial": {"active_holders": active_holders, "passive_summary": passive_summary}}

            has_whale_accumulation = any(h.get("type") in ("Активист-акционер 🎯", "Стратегический инвестор") and h.get("pct_change", 0) > 0 for h in active_holders)
            if has_whale_accumulation and 60 <= weighted_score <= 64:
                weighted_score += 5

            if weighted_score >= 80: letter_grade = "A"
            elif weighted_score >= 65: letter_grade = "B"
            elif weighted_score >= 50: letter_grade = "C"
            else: letter_grade = "D"
            ...
            try:
                leverage_target_debt = debt if debt > 0 else mcap * 0.30
                covenants_profile = simulate_negative_covenants(post_trans_debt=leverage_target_debt, post_trans_ebitda=ebitda_pre, net_income=float(latest.get("net_income", 0.0)), capex=float(latest.get("capex", 0.0)), da=float(latest.get("da", 0.0)), buyer_type="financial")
            except Exception as e:
            ...
            comp_obj = {
                ...
                "financials": {
                    ...
                    "fiduciary_ceiling": round(fiduciary_ceiling, 2),
                    "cvr_earnout_per_share": round(cvr_earnout_required_per_share, 2),
                    "is_hubris_trap": is_hubris_trap,
